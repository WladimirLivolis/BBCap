...
TAIL_SIZE = 0 # This packet MUST have size ZERO
...
2. Sends 2 groups of packet trains to destination:
    2.1 GROUP_1:
        2.1.1 For each packet train of GROUP_1:
            ...
            2.1.1.3 ONE packet (TAIL)*** is sent: SIZE = ZERO & TTL = K;
            2.1.1.4 Returns RTT = time between the sending of the locomotive and the response to the tail;
    ...
    2.2 GROUP_2:
        2.2.1 For each packet train of GROUP_2:
            ...
            2.2.1.3 ONE packet (TAIL)*** is sent: SIZE = ZERO & TTL = K;
            2.2.1.4 Returns RTT = time between the sending of the locomotive and the response to the tail;
...
*   Locomotive packets do not cause response either because TTL < K or because they are malformed.
**  Car packets do not cause response as they are malformed.
*** Tail packets have same size as their headers.
------------------------------------------------------------------------
'''))
...
printer("TRAIN = LOCOMOTIVE + CARS + TAIL\n",output)
...
printer("TAIL --> Length = "+str(TAIL_SIZE)+"B | TTL = "+str(K)+" hops\n",output)
RTT1 = LatencyTools().latency_tester(DESTINATION_NAME, LOCOMOTIVE_SIZE, K-1, GROUP1_CAR_SIZE, K, TAIL_SIZE, K, NUMBER_OF_TRAINS, NUMBER_OF_CARS, output)

printer("\n==================== Group #2 ====================",output)
printer("\nLOCOMOTIVE --> Length = "+str(LOCOMOTIVE_SIZE)+"B | TTL = "+str(K-1)+" hops",output)
printer("CAR --> Length = "+str(GROUP2_CAR_SIZE)+"B | TTL = "+str(K)+" hops",output)
printer("TAIL --> Length = "+str(TAIL_SIZE)+"B | TTL = "+str(K)+" hops\n",output)
RTT2 = LatencyTools().latency_tester(DESTINATION_NAME, LOCOMOTIVE_SIZE, K-1, GROUP2_CAR_SIZE, K, TAIL_SIZE, K, NUMBER_OF_TRAINS, NUMBER_OF_CARS, output)
